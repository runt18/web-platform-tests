...
def write_novalid_files():
    for el, attr in (pair.split() for pair in element_attribute_pairs):
        for desc, url in invalid.items():
            if ("area" == el):
                f = open(os.path.join(ccdir, "html/elements/area/href/{0!s}-novalid.html".format(desc)), 'wb')
                f.write(template + '<title>invalid href: {0!s}</title>\n'.format(desc))
                f.write('<map name=foo><{0!s} {1!s}="{2!s}" alt></map>\n'.format(el, attr, url))
                f.close()
            elif ("base" == el or "embed" == el):
                f = open(os.path.join(ccdir, "html/elements/{0!s}/{1!s}/{2!s}-novalid.html".format(el, attr, desc)), 'wb')
                f.write(template + '<title>invalid {0!s}: {1!s}</title>\n'.format(attr, desc))
                f.write('<{0!s} {1!s}="{2!s}">\n'.format(el, attr, url))
                f.close()
            elif ("html" == el):
                f = open(os.path.join(ccdir, "html/elements/html/manifest/{0!s}-novalid.html".format(desc)), 'wb')
                f.write('<!DOCTYPE html>\n')
                f.write('<html manifest="{0!s}">\n'.format(url))
                f.write('<meta charset=utf-8>\n')
                f.write('<title>invalid manifest: {0!s}</title>\n'.format(desc))
                f.write('</html>\n')
                f.close()
            elif ("img" == el):
                f = open(os.path.join(ccdir, "html/elements/img/src/{0!s}-novalid.html".format(desc)), 'wb')
                f.write(template + '<title>invalid src: {0!s}</title>\n'.format(desc))
                f.write('<img src="{0!s}" alt>\n'.format(url))
                f.close()
            elif ("input" == el and "src" == attr):
                f = open(os.path.join(ccdir, "html/elements/input/type-image-src/{0!s}-novalid.html".format(desc)), 'wb')
                f.write(template + '<title>invalid src: {0!s}</title>\n'.format(desc))
                f.write('<{0!s} type=image alt="foo" {1!s}="{2!s}">\n'.format(el, attr, url))
                f.close()
            elif ("input" == el and "formaction" == attr):
                f = open(os.path.join(ccdir, "html/elements/input/type-submit-formaction/{0!s}-novalid.html".format(desc)), 'wb')
                f.write(template + '<title>invalid formaction: {0!s}</title>\n'.format(desc))
                f.write('<{0!s} type=submit {1!s}="{2!s}">\n'.format(el, attr, url))
                f.close()
                f = open(os.path.join(ccdir, "html/elements/input/type-image-formaction/{0!s}-novalid.html".format(desc)), 'wb')
                f.write(template + '<title>invalid formaction: {0!s}</title>\n'.format(desc))
                f.write('<{0!s} type=image alt="foo" {1!s}="{2!s}">\n'.format(el, attr, url))
                f.close()
            elif ("input" == el and "value" == attr):
                f = open(os.path.join(ccdir, "html/elements/input/type-url-value/{0!s}-novalid.html".format(desc)), 'wb')
                f.write(template + '<title>invalid value attribute: {0!s}</title>\n'.format(desc))
                f.write('<{0!s} type=url {1!s}="{2!s}">\n'.format(el, attr, url))
                f.close()
            elif ("link" == el):
                f = open(os.path.join(ccdir, "html/elements/link/href/{0!s}-novalid.html".format(desc)), 'wb')
                f.write(template + '<title>invalid href: {0!s}</title>\n'.format(desc))
                f.write('<link href="{0!s}" rel=help>\n'.format(url))
                f.close()
            elif ("source" == el or "track" == el):
                f = open(os.path.join(ccdir, "html/elements/{0!s}/{1!s}/{2!s}-novalid.html".format(el, attr, desc)), 'wb')
                f.write(template + '<title>invalid {0!s}: {1!s}</title>\n'.format(attr, desc))
                f.write('<video><{0!s} {1!s}="{2!s}"></video>\n'.format(el, attr, url))
                f.close()
            else:
                f = open(os.path.join(ccdir, "html/elements/{0!s}/{1!s}/{2!s}-novalid.html".format(el, attr, desc)), 'wb')
                f.write(template + '<title>invalid {0!s}: {1!s}</title>\n'.format(attr, desc))
                f.write('<{0!s} {1!s}="{2!s}"></{3!s}>\n'.format(el, attr, url, el))
                f.close()
    for desc, url in invalid.items():
        f = open(os.path.join(ccdir, "html/microdata/itemid/{0!s}-novalid.html".format(desc)), 'wb')
        f.write(template + '<title>invalid itemid: {0!s}</title>\n'.format(desc))
        f.write('<div itemid="{0!s}" itemtype="http://foo" itemscope></div>\n'.format(url))
        f.close()
    for desc, url in invalid_absolute.items():
        f = open(os.path.join(ccdir, "html/microdata/itemtype/{0!s}-novalid.html".format(desc)), 'wb')
        f.write(template + '<title>invalid itemtype: {0!s}</title>\n'.format(desc))
        f.write('<div itemtype="{0!s}" itemscope></div>\n'.format(url))
        f.close()
        f = open(os.path.join(ccdir, "html/elements/input/type-url-value/{0!s}-novalid.html".format(desc)), 'wb')
        f.write(template + '<title>invalid value attribute: {0!s}</title>\n'.format(desc))
        f.write('<input type=url value="{0!s}">\n'.format(url))
        f.close()
...
def write_haswarn_files():
    for el, attr in (pair.split() for pair in element_attribute_pairs):
        for desc, url in warnings.items():
            if ("area" == el):
                f = open(os.path.join(ccdir, "html/elements/area/href/{0!s}-haswarn.html".format(desc)), 'wb')
                f.write(template + '<title>{0!s} warning: {1!s}</title>\n'.format(attr, desc))
                f.write('<map name=foo><{0!s} {1!s}="{2!s}" alt></map>\n'.format(el, attr, url))
                f.close()
            elif ("base" == el or "embed" == el):
                f = open(os.path.join(ccdir, "html/elements/{0!s}/{1!s}/{2!s}-haswarn.html".format(el, attr, desc)), 'wb')
                f.write(template + '<title>{0!s} warning: {1!s}</title>\n'.format(attr, desc))
                f.write('<{0!s} {1!s}="{2!s}">\n'.format(el, attr, url))
                f.close()
            elif ("html" == el):
                f = open(os.path.join(ccdir, "html/elements/html/manifest/{0!s}-haswarn.html".format(desc)), 'wb')
                f.write('<!DOCTYPE html>\n')
                f.write('<html manifest="{0!s}">\n'.format(url))
                f.write('<meta charset=utf-8>\n')
                f.write('<title>{0!s} warning: {1!s}</title>\n'.format(attr, desc))
                f.write('</html>\n')
                f.close()
            elif ("img" == el):
                f = open(os.path.join(ccdir, "html/elements/img/src/{0!s}-haswarn.html".format(desc)), 'wb')
                f.write(template + '<title>{0!s} warning: {1!s}</title>\n'.format(attr, desc))
                f.write('<{0!s} {1!s}="{2!s}" alt>\n'.format(el, attr, url))
                f.close()
            elif ("input" == el and "src" == attr):
                f = open(os.path.join(ccdir, "html/elements/input/type-image-src/{0!s}-haswarn.html".format(desc)), 'wb')
                f.write(template + '<title>{0!s} warning: {1!s}</title>\n'.format(attr, desc))
                f.write('<{0!s} type=image alt="foo" {1!s}="{2!s}">\n'.format(el, attr, url))
                f.close()
            elif ("input" == el and "formaction" == attr):
                f = open(os.path.join(ccdir, "html/elements/input/type-submit-formaction/{0!s}-haswarn.html".format(desc)), 'wb')
                f.write(template + '<title>{0!s} warning: {1!s}</title>\n'.format(attr, desc))
                f.write('<{0!s} type=submit {1!s}="{2!s}">\n'.format(el, attr, url))
                f.close()
                f = open(os.path.join(ccdir, "html/elements/input/type-image-formaction/{0!s}-haswarn.html".format(desc)), 'wb')
                f.write(template + '<title>{0!s} warning: {1!s}</title>\n'.format(attr, desc))
                f.write('<{0!s} type=image alt="foo" {1!s}="{2!s}">\n'.format(el, attr, url))
                f.close()
            elif ("input" == el and "value" == attr):
                f = open(os.path.join(ccdir, "html/elements/input/type-url-value/{0!s}-haswarn.html".format(desc)), 'wb')
                f.write(template + '<title>{0!s} warning: {1!s}</title>\n'.format(attr, desc))
                f.write('<{0!s} type=url {1!s}="{2!s}">\n'.format(el, attr, url))
                f.close()
            elif ("link" == el):
                f = open(os.path.join(ccdir, "html/elements/link/href/{0!s}-haswarn.html".format(desc)), 'wb')
                f.write(template + '<title>{0!s} warning: {1!s}</title>\n'.format(attr, desc))
                f.write('<{0!s} {1!s}="{2!s}" rel=help>\n'.format(el, attr, url))
                f.close()
            elif ("source" == el or "track" == el):
                f = open(os.path.join(ccdir, "html/elements/{0!s}/{1!s}/{2!s}-haswarn.html".format(el, attr, desc)), 'wb')
                f.write(template + '<title>{0!s} warning: {1!s}</title>\n'.format(attr, desc))
                ...
                f.close()
    for desc, url in warnings.items():
        f = open(os.path.join(ccdir, "html/microdata/itemtype-{0!s}-haswarn.html".format(desc) ), 'wb')
        f.write(template + '<title>warning: {0!s}</title>\n'.format(desc))
        f.write('<div itemtype="{0!s}" itemscope></div>\n'.format(url))
        f.close()
        f = open(os.path.join(ccdir, "html/microdata/itemid-{0!s}-haswarn.html".format(desc)), 'wb')
        f.write(template + '<title>warning: {0!s}</title>\n'.format(desc))
        f.write('<div itemid="{0!s}" itemtype="http://foo" itemscope></div>\n'.format(url))
        f.close()
...
def write_isvalid_files():
    for el, attr in (pair.split() for pair in element_attribute_pairs):
        ...
        if ("html" == el):
            ...
            f = open(os.path.join(ccdir, "html/elements/{0!s}/{1!s}-isvalid.html".format(el, attr)), 'wb')
            f.write(template + '<title>valid {0!s}</title>\n'.format(attr))
        for desc, url in valid.items():
            if ("area" == el):
                f.write('<map name=foo><{0!s} {1!s}="{2!s}" alt></map><!-- {3!s} -->\n'.format(el, attr, url, desc))
            elif ("embed" == el):
                f.write('<{0!s} {1!s}="{2!s}"><!-- {3!s} -->\n'.format(el, attr, url, desc))
            elif ("img" == el):
                f.write('<{0!s} {1!s}="{2!s}" alt><!-- {3!s} -->\n'.format(el, attr, url, desc))
            elif ("input" == el and "src" == attr):
                f.write('<{0!s} type=image alt="foo" {1!s}="{2!s}"><!-- {3!s} -->\n'.format(el, attr, url, desc))
            elif ("input" == el and "formaction" == attr):
                fs.write('<{0!s} type=submit {1!s}="{2!s}"><!-- {3!s} -->\n'.format(el, attr, url, desc))
                fi.write('<{0!s} type=image alt="foo" {1!s}="{2!s}"><!-- {3!s} -->\n'.format(el, attr, url, desc))
            elif ("link" == el):
                f.write('<{0!s} {1!s}="{2!s}" rel=help><!-- {3!s} -->\n'.format(el, attr, url, desc))
            elif ("source" == el or "track" == el):
                f.write('<video><{0!s} {1!s}="{2!s}"></video><!-- {3!s} -->\n'.format(el, attr, url, desc))
            else:
                f.write('<{0!s} {1!s}="{2!s}"></{3!s}><!-- {4!s} -->\n'.format(el, attr, url, el, desc))
        if ("input" == el and "formaction" == attr):
            ...
            f.close()
    for desc, url in valid.items():
        f = open(os.path.join(ccdir, "html/elements/base/href/{0!s}-isvalid.html".format(desc)), 'wb')
        f.write(template + '<title>valid href: {0!s}</title>\n'.format(desc))
        f.write('<base href="{0!s}">\n'.format(url))
        f.close()
        f = open(os.path.join(ccdir, "html/elements/html/manifest/{0!s}-isvalid.html".format(desc)), 'wb')
        f.write('<!DOCTYPE html>\n')
        f.write('<html manifest="{0!s}">\n'.format(url))
        f.write('<meta charset=utf-8>\n')
        f.write('<title>valid manifest: {0!s}</title>\n'.format(desc))
        f.write('</html>\n')
        f.close()
    f = open(os.path.join(ccdir, "html/elements/meta/refresh-isvalid.html"), 'wb')
    f.write(template + '<title>valid meta refresh</title>\n')
    for desc, url in valid.items():
        f.write('<meta http-equiv=refresh content="0; URL={0!s}"><!-- {1!s} -->\n'.format(url, desc))
    f.close()
    f = open(os.path.join(ccdir, "html/microdata/itemid-isvalid.html"), 'wb')
    f.write(template + '<title>valid itemid</title>\n')
    for desc, url in valid.items():
        f.write('<div itemid="{0!s}" itemtype="http://foo" itemscope></div><!-- {1!s} -->\n'.format(url, desc))
    f.close()
    ...
    for desc, url in valid_absolute.items():
        f.write('<div itemtype="{0!s}" itemscope></div><!-- {1!s} -->\n'.format(url, desc))
    f.close()
    ...
    for desc, url in valid_absolute.items():
        f.write('<input type=url value="{0!s}"><!-- {1!s} -->\n'.format(url, desc))
    f.close()
